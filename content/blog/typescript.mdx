...
description: A practical walkthrough of TypeScript from primitives to generics, narrowing and discriminated unions, with runnable examples for each idea.
date: 2024-09-23
published: true
category: TypeScript
...
Type: A type alias has no `extends` clause. You build on one with an intersection instead, and a class can `implement` a type alias as long as it names an object shape rather than a union.

```typescript {1,2,4,6,7,8} showLineNumbers
type A = { prop: number };
type B = { other: string };

type C = A & B; // Intersection, the alias way to extend

class Impl implements A {
  prop = 1; // A class can implement an object type alias
}
```
